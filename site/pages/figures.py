...
def bar_sheet() -> str:
    rows = [("same-day", 50, "$4 / $4", "var(--sage)"),
            ("codeshare", 80, "$9 / $36", "var(--accent)"),
            ("refund, unheld", 98, "$12 / $600", "var(--stop)"),
            ("refund, with a hold", 71, "$12 / $30", "var(--ok)")]
    ...
    for i, (name, bar, costs, colour) in enumerate(rows):
        ...
        if w > 235:
            out.append(f'<text x="{144+w:.0f}" y="{y+15}" text-anchor="end" font-size="12" '
                       f'font-weight="700" fill="#fff">{bar}%</text>')
        else:
    ...
    out.append('<path d="M462 108 C496 108 496 144 402 144" fill="none" stroke="var(--ok)" '
               'stroke-width="1.6" stroke-dasharray="3 3"/>')
    out.append('<text x="498" y="130" text-anchor="middle" font-size="11" fill="var(--ok)" '
...
def chain() -> str:
    ...
    x = 12
    for i in range(6):
        out.append(f'<rect x="{x}" y="34" width="56" height="34" rx="5" fill="var(--accent)" '
                   f'fill-opacity=".14" stroke="var(--accent)"/>')
        out.append(f'<text x="{x+28}" y="56" text-anchor="middle" font-size="12" fill="var(--accent)">0.9</text>')
        if i < 5:
    ...
    for n in range(1, 7):
        ...
        out.append(f'<circle cx="{px}" cy="{py:.0f}" r="3.4" fill="var(--stop)"/>')
        out.append(f'<text x="{px}" y="{py-8:.0f}" text-anchor="middle" font-size="11" '
                   f'font-weight="600" fill="var(--stop)">{p*100:.0f}%</text>')
    out.append(f'<polyline points="{" ".join(pts)}" fill="none" stroke="var(--stop)" stroke-width="1.8"/>')
...
def cache_prefix() -> str:
    blocks = [("tools", 96, "var(--accent)"), ("system", 88, "var(--accent)"),
              ("shared context", 120, "var(--accent)"), ("domain context", 116, "var(--accent)")]
    out, x = [], 12
    for name, w, colour in blocks:
        ...
        x += w + 6
    out.append(f'<line x1="{x-3}" y1="34" x2="{x-3}" y2="100" stroke="var(--warn)" stroke-width="2.5"/>')
    out.append(f'<text x="{x-3}" y="28" text-anchor="middle" font-size="10.5" font-weight="700" '
               f'fill="var(--warn)">cache marker</text>')
    out.append(f'<rect x="{x+4}" y="46" width="{540-x}" height="40" rx="4" fill="var(--stop)" '
               f'fill-opacity=".13" stroke="var(--stop)" stroke-dasharray="4 3"/>')
    out.append(f'<text x="{x+4+(540-x)/2:.0f}" y="71" text-anchor="middle" font-size="11.5" '
               f'fill="var(--stop)">the request</text>')
    ...
    out.append('<text x="12" y="168" font-size="11.5" fill="var(--stop)" opacity=".9">'
...
def bolt_days() -> str:
    bolts = [("walking skeleton", 0, "var(--sage)"), ("fare maths", 1, "var(--accent)"),
             ("eligibility rules", 2, "var(--accent)"), ("ranking", 3, "var(--accent)"),
             ("checker", 4, "var(--accent)"), ("MCP server", 5, "var(--ochre)"),
             ("rebook (gated)", 6, "var(--ochre)"), ("refund (gated)", 7, "var(--stop)"),
             ("shadow path", 8, "var(--plum)"), ("harness", 9, "var(--plum)")]
    out = []
    ...
    out.append('<text x="12" y="176" font-size="11" fill="var(--stop)" opacity=".85">'
...
def shadow_widen() -> str:
    stages = [("shadow", "decides, never acts", 120, "var(--soft)"),
              ("5%", "12 cases a day", 90, "var(--warn)"),
              ("25%", "60 a day", 110, "var(--accent)"),
              ("100%", "widened on evidence", 150, "var(--ok)")]
    out, x = [], 12
    ...
    out.append('<text x="12" y="162" font-size="11.5" fill="var(--stop)" opacity=".9">'
...
def bill_factors() -> str:
    ...
    for name, v, sub in facs:
        out.append(f'<rect x="{x}" y="40" width="112" height="56" rx="6" fill="var(--warn)" fill-opacity=".16" '
                   f'stroke="var(--warn)"/>')
    ...
    out.append('<text x="276" y="128" text-anchor="middle" font-size="18" font-weight="700" '
               'fill="var(--stop)">= 4.4× the estimate, on flat traffic</text>')
...
def authority_ladder() -> str:
    rows = [("R1", "search_flights", "read only", "nothing", "#7C8596"),
            ("R2", "draft_message", "reversible", "one reader", "#5B7FA8"),
            ("R3", "rebook(...)", "hard to reverse", "approve first", "#8C7A5B"),
            ("R4", "issue_refund(≤400)", "money", "named approver", "#B0603A"),
            ("R5", "change_identity", "irreversible", "not delegated", "#8C3B3B")]
    out = []
...
def two_numbers() -> str:
    ...
    rows = [("person-days per story", "8.0", "4.6", "−43%", "var(--ok)"),
            ("token spend per story", "—", "$310", "", "var(--warn)"),
            ("review hours added", "1.2", "2.0", "+0.8", "var(--warn)"),
    ...
    out.append('<text x="12" y="176" font-size="11.5" fill="var(--stop)" opacity=".9">'
